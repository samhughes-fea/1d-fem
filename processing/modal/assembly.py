...
def _compute_local_global_dof_map(elements: List, total_dof: int) -> List[np.ndarray]:
    """Compute global DOF indices for each element (same convention as static)."""
...
def assemble_global_matrices(
    ...
    ----------
    elements : list
        Element objects with assemble_global_dof_indices().
    element_stiffness_matrices : list, optional
        Per-element K_e (dense or sparse).
    element_mass_matrices : list, optional
        Per-element M_e (dense or sparse).
    ...
    return K_global, M_global, local_global_dof_map
