    ...
    **N tensor:** per-Gauss shape-function slice is ``(12, 6)``; load projection uses the standard 12-DOF layout,
    ...
    def __init__(
        ...
        *,
        ...
        # Quadrature order: from integration_orders when not provided (axial, shear, torsion)
        if quadrature_order is not None:
            ...
            self.quadrature_order = max(axial_order, shear_y_order, shear_z_order, torsion_order, 1)
    ...
    def _build_shape_function_coefficients_b2(
        ...
        """B2 coefficient arrays (12, 6, 4) for N, dN/dξ, d²N/dξ². Truss: linear Lagrange for u_x, u_y, u_z, θ_x."""
        ...
        dc[0, 0, 0], dc[6, 0, 0] = -0.5, 0.5
        # comp 1 (u_y): DOF 1, 7
        c[1, 1, 0], c[1, 1, 1] = 0.5, -0.5
        c[7, 1, 0], c[7, 1, 1] = 0.5, 0.5
        dc[1, 1, 0], dc[7, 1, 0] = -0.5, 0.5
        # comp 2 (u_z): DOF 2, 8
        c[2, 2, 0], c[2, 2, 1] = 0.5, -0.5
        c[8, 2, 0], c[8, 2, 1] = 0.5, 0.5
        dc[2, 2, 0], dc[8, 2, 0] = -0.5, 0.5
        # comp 3 (θ_x): DOF 3, 9
        c[3, 3, 0], c[3, 3, 1] = 0.5, -0.5
        c[9, 3, 0], c[9, 3, 1] = 0.5, 0.5
        dc[3, 3, 0], dc[9, 3, 0] = -0.5, 0.5
        return c, dc, d2c
    ...
    def element_stiffness_matrix(self):
        ...
        if self.logger_operator:
            self.logger_operator.log_matrix("stiffness", Ke, {"name": "Truss Element Stiffness"})
