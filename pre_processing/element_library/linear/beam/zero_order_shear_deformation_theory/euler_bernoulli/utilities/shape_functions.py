...
class ShapeFunctionOperator:
    ...
    **Shape function basis and active entries of N_g**
    ...
    def build_shape_function_coefficients_b2(
        self,
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        B2 coefficient arrays (12, 6, 4) for N, dN/dξ, d²N/dξ² in monomial basis ξ^0..ξ^3.

        Same polynomials as ``natural_coordinate_form``; used for serialized / evaluated
        post-processing (see ``ElementObject`` shape_function_*_coefficients).

        Returns
        -------
        c, dc, d2c : np.ndarray
            Each float64 array of shape (12, 6, 4): DOF × component × monomial power.
        """
        L = self.element_length
