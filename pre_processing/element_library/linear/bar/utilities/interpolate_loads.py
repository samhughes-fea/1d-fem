...
class LoadInterpolationOperator:
    """
    Interpolates distributed loads for bar/truss 6-DOF elements.
    Same API as beam: array shape (N, 9) [x, y, z, Fx, Fy, Fz, Mx, My, Mz],
    interpolate(x_phys) returns (M, 6) [Fx, Fy, Fz, Mx, My, Mz].

    Parameters
    ----------
    distributed_loads_array : np.ndarray, shape (N, 9)
        Columns: [x, y, z, Fx, Fy, Fz, Mx, My, Mz].
    ...
    def _validate_interpolation_order(self) -> None:
        ...
        if not valid_orders:
            raise ValueError(
                f"No valid interpolation scheme for {n_points} points and "
                f"{self.n_gauss_points} Gauss points."
            )
        if self.interpolation_order not in valid_orders:
            object.__setattr__(self, 'interpolation_order', valid_orders[-1])
    ...
    def _build_interpolators(self) -> None:
        ...
        for i, is_active in enumerate(self._active_components):
            ...
            fill_val = (y[0], y[-1]) if self.boundary_mode == "clamp" else (
                0.0 if self.boundary_mode == "zero" else None
            )
    ...
    def interpolate(self, x_phys: np.ndarray) -> np.ndarray:
        """
        Interpolate [Fx, Fy, Fz, Mx, My, Mz] at positions x_phys.
        Returns shape (M, 6) or (6,) if single point.
        """
        ...
        for i, is_active in enumerate(self._active_components):
            if is_active:
                try:
                    ...
                    raise ValueError(
                        f"Interpolation failed for component {i} at positions {x_phys}. "
                        f"Valid range: [{x_min:.3e}, {x_max:.3e}]. {e}"
                    ) from e
