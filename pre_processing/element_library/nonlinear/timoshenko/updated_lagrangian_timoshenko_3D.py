# pre_processing/element_library/nonlinear/timoshenko/updated_lagrangian_timoshenko_3D.py
"""
...
**Strain/stress (planned implementation):** Not the TL Green–Lagrange / reference-\\(X\\) pipeline in ``nonlinear/timoshenko/utilities/green_lagrange_strain.py``; UL uses small-strain measures on the **updated** configuration (see ``docs/conventions/FORMULATION_DOCSTRING_STANDARDS.md``).
...
Limitations
-----------
The Updated-Lagrangian utilities are not yet provided; ``_get_K_0_current()`` is a
docstring-only marker that the material/geometric tangents must be evaluated about the
current configuration.  All assembly methods raise ``NotImplementedError``.
