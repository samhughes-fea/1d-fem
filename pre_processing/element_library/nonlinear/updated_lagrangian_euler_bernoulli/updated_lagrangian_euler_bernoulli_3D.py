# pre_processing/element_library/nonlinear/updated_lagrangian_euler_bernoulli/updated_lagrangian_euler_bernoulli_3D.py
"""
...
Limitations
-----------
The Updated-Lagrangian utilities tree (``nonlinear/updated_lagrangian/utilities/`` —
Almansi or Cauchy strain operator, current-configuration ``K_sigma``, configuration
update API) is not yet provided; ``_get_K_0_current()`` is a docstring-only marker
that the material/geometric tangents must be evaluated about the current configuration
(see ``Notes``).  All assembly methods raise ``NotImplementedError`` until that tree
lands.
