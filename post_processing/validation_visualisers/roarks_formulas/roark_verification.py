# post_processing/validation_visualisers/roarks_formulas/roark_verification.py
"""
...
def run_roark_verification(scale: float = 1.0) -> None:
    ...
    for csv_path in csv_files:
        ...
        if m:
            jid = int(m.group("id"))
            if jid not in job_to_result:
                job_to_result[jid] = (csv_file, job_dir)
    ...
    for row, (job_id, title, a_frac) in enumerate(POINT_LOAD_CASES):
        ...
        grid_file = jobs_dir / f"job_{job_id:04d}" / "grid.txt"
