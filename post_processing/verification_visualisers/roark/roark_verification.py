...
E: Final[float] = 2.1e11   # Pa (match job material and deformation_convergence / gci_richardson_roark_report)
...
def run_roark_verification(scale: float = 1.0) -> None:
    ...
    for row, (job_id, title, a_frac) in enumerate(POINT_LOAD_CASES):
        ...
        axes_point[row, 0].plot(x, uy_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
        axes_point[row, 0].plot(x_ana, uy_ana, "k-", linewidth=0.5, label="Roark")
        axes_point[row, 0].set_ylabel(r"$u_y$ [mm]")
        axes_point[row, 0].set_title(title, fontweight="bold")
        axes_point[row, 0].grid(ls="--", alpha=0.6)
        axes_point[row, 0].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")

        axes_point[row, 1].plot(x, th_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
        axes_point[row, 1].plot(x_ana, th_ana, "k-", linewidth=0.5, label="Roark")
        ...
        axes_point[row, 1].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")
    ...
    fig_point.subplots_adjust(top=0.92, bottom=0.28)
    fig_point.savefig(out_dir / "roark_verification_point_loads_euler_bernoulli.png", dpi=300)
    plt.close(fig_point)
    print(f"Saved: {out_dir / 'roark_verification_point_loads_euler_bernoulli.png'}")
    ...
    for row, (job_id, title, roark_type) in enumerate(DIST_LOAD_CASES):
        ...
        axes_dist[row, 0].plot(x, uy_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
        axes_dist[row, 0].plot(x_ana, uy_ana, "k-", linewidth=0.5, label="Roark")
        axes_dist[row, 0].set_ylabel(r"$u_y$ [mm]")
        axes_dist[row, 0].set_title(title, fontweight="bold")
        axes_dist[row, 0].grid(ls="--", alpha=0.6)
        axes_dist[row, 0].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")

        axes_dist[row, 1].plot(x, th_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
        axes_dist[row, 1].plot(x_ana, th_ana, "k-", linewidth=0.5, label="Roark")
        axes_dist[row, 1].set_ylabel(r"$\theta_z$ [deg]")
        axes_dist[row, 1].set_title(title, fontweight="bold")
        axes_dist[row, 1].grid(ls="--", alpha=0.6)
        axes_dist[row, 1].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")
    ...
    fig_dist.subplots_adjust(top=0.92, bottom=0.28)
    fig_dist.savefig(out_dir / "roark_verification_distributed_loads_euler_bernoulli.png", dpi=300)
    plt.close(fig_dist)
    print(f"Saved: {out_dir / 'roark_verification_distributed_loads_euler_bernoulli.png'}")
    ...
    if any(job_id in job_to_result for job_id, _, _ in POINT_LOAD_CASES_TIMS + DIST_LOAD_CASES_TIMS):
        ...
        for row, (job_id, title, load_type) in enumerate(POINT_LOAD_CASES_TIMS):
            ...
            uy_ana = roark_fine["deflection"] * 1000
            th_ana = np.degrees(roark_fine["rotation"])
            axes_pt[row, 0].plot(x, uy_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
            axes_pt[row, 0].plot(x_ana, uy_ana, "k-", linewidth=0.5, label="Roark Timoshenko")
            axes_pt[row, 0].set_ylabel(r"$u_y$ [mm]")
            axes_pt[row, 0].set_title(title, fontweight="bold")
            axes_pt[row, 0].grid(ls="--", alpha=0.6)
            axes_pt[row, 0].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")
            axes_pt[row, 1].plot(x, th_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
            axes_pt[row, 1].plot(x_ana, th_ana, "k-", linewidth=0.5, label="Roark Timoshenko")
            axes_pt[row, 1].set_ylabel(r"$\theta_z$ [deg]")
            axes_pt[row, 1].set_title(title, fontweight="bold")
            axes_pt[row, 1].grid(ls="--", alpha=0.6)
            axes_pt[row, 1].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")
        axes_pt[-1, 0].set_xlabel(r"$x$ [m]")
        axes_pt[-1, 1].set_xlabel(r"$x$ [m]")
        fig_pt.tight_layout()
        fig_pt.subplots_adjust(top=0.92, bottom=0.28)
        ...
        for row, (job_id, title, roark_type) in enumerate(DIST_LOAD_CASES_TIMS):
            ...
            uy_ana = np.interp(x_ana, x_ana_sorted, roark_fine["deflection"] * 1000)
            th_ana = np.interp(x_ana, x_ana_sorted, np.degrees(roark_fine["rotation"]))
            axes_dt[row, 0].plot(x, uy_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
            axes_dt[row, 0].plot(x_ana, uy_ana, "k-", linewidth=0.5, label="Roark Timoshenko")
            axes_dt[row, 0].set_ylabel(r"$u_y$ [mm]")
            axes_dt[row, 0].set_title(title, fontweight="bold")
            axes_dt[row, 0].grid(ls="--", alpha=0.6)
            axes_dt[row, 0].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")
            axes_dt[row, 1].plot(x, th_fem, color=COLORS[row], linestyle="--", linewidth=1.25, label=f"FEM job_{job_id:04d}")
            axes_dt[row, 1].plot(x_ana, th_ana, "k-", linewidth=0.5, label="Roark Timoshenko")
            axes_dt[row, 1].set_ylabel(r"$\theta_z$ [deg]")
            axes_dt[row, 1].set_title(title, fontweight="bold")
            axes_dt[row, 1].grid(ls="--", alpha=0.6)
            axes_dt[row, 1].legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), fontsize="small")
        axes_dt[-1, 0].set_xlabel(r"$x$ [m]")
        axes_dt[-1, 1].set_xlabel(r"$x$ [m]")
        fig_dt.tight_layout()
        fig_dt.subplots_adjust(top=0.92, bottom=0.28)
