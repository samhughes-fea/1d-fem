# Design note: nonlinear prestress for linear modal buckling

**Implemented:** `modal.buckling_prestress = nonlinear_static` (see `ModalSimulationRunner._run_linear_buckling`).
...
### Option B — `modal.buckling_nonlinear_prestress_twins = true`

When **`buckling_prestress = nonlinear_static`** and **`buckling_nonlinear_prestress_twins`** is enabled, `ModalSimulationRunner` builds a **parallel** element array with registered nonlinear twins (`LinearTimoshenkoBeamElement3D` → `NonlinearTimoshenkoBeamElement3D`, `LinearEulerBernoulliBeamElement3D` → `NonlinearEulerBernoulliBeamElement3D`), runs `NonlinearStaticSimulationRunner` on those instances, then assembles **`K_σ`** with the **original** linear elements from `run_job` (same global DOF layout).
...
- [x] Optional twins: `modal.buckling_nonlinear_prestress_twins` with nonlinear twin instantiation (`simulation_runner/modal/modal_simulation.py`).
