...
- **Stagewise operations (spectral / harmonic / transient):** Runners compose class-based stages (same style as **`processing.static.operations`**). **§2 / §5 spectral:** [`processing.spectral.operations`](../../processing/spectral/operations/__init__.py) — `PrepareSpectralLocalMatrices`, `AssembleSpectralGlobalSystem`, `ModifySpectralGlobalSystem`, `SolveGeneralizedEigenproblem`, plus buckling-only `AssembleBucklingGeometricStiffness`, `ModifyBucklingGlobalMatrices`, `SolveLinearBucklingEigenpairs` — wrap the **`processing.eigen`** / **`processing.buckling`** kernels and write optional per-stage logs under the job tree. [`VibrationBucklingBackend`](../../simulation_runner/spectral/vibration_buckling_backend.py) orchestrates these with **`RuntimeMonitorTelemetry`**. **§4 harmonic:** [`processing.harmonic.operations`](../../processing/harmonic/operations/__init__.py). **§3 transient:** [`processing.dynamic.operations`](../../processing/dynamic/operations/__init__.py) (no imports from **`processing.static`** in **`processing.dynamic`**). Per-stage file logging for those operation classes uses **`processing.common.stage_logging.init_stage_logger`**. Global spectral matrix logging lives in **`processing.spectral.spectral_diagnostics`** (also re-exported from **`simulation_runner.spectral.spectral_diagnostics`**).
- **Runner-level modular API (non-static):** [`VibrationBucklingBackend`](../../simulation_runner/spectral/vibration_buckling_backend.py), [`TransientSimulationRunner`](../../simulation_runner/transient/dynamic_simulation.py), and [`HarmonicSimulationRunner`](../../simulation_runner/harmonic/harmonic_simulation.py) expose **instance methods** named after their mathematical stages (prepare / assemble / modify / solve / save) so call graphs mirror [`LinearStaticSimulationRunner`](../../simulation_runner/static/linear_static_simulation.py). Mapping table (including **N/A** static stages such as condensation): [`simulation_runner/spectral/README.md`](../../simulation_runner/spectral/README.md) (*Stage mapping vs linear static*). **Processing** stage classes in spectral/harmonic/dynamic use **`run()`** as the common entry point; static operations keep **`assemble()`** / **`apply_*()`** names — see [`processing/spectral/README.md`](../../processing/spectral/README.md).
- **Linear buckling (§5):** When `type` is **`buckling`** and **`buckling.nonlinear_buckling`** is false (default; legacy: `modal` + `modal.analysis = buckling`), [`LinearBucklingSimulationRunner`](../../simulation_runner/buckling/buckling_simulation.py) / shared spectral pipeline builds prestress displacements via **`linear_static`** (default) or **`nonlinear_static`** (`buckling.buckling_prestress` or legacy `modal.buckling_prestress`), then assembles elastic **`K`** and stress-based geometric stiffness **`K_σ`**, then solves the linear buckling eigenproblem. When **`nonlinear_buckling`** is true, [`NonlinearBucklingSimulationRunner`](../../simulation_runner/buckling/nonlinear_buckling_simulation.py) is used instead (MVP stub — see [`NONLINEAR_BUCKLING_MVP.md`](NONLINEAR_BUCKLING_MVP.md)). **`nonlinear_static`** prestress requires nonlinear beam elements and a converged nonlinear prestress solve; see [`MODAL_BUCKLING_NONLINEAR_PRESTRESS_DESIGN.md`](MODAL_BUCKLING_NONLINEAR_PRESTRESS_DESIGN.md). **Warping meshes (7 DOF per node)** are supported for linear EB/Timoshenko: **`K_σ`** embeds the same **12×12** beam-column geometric stiffness on standard translation/rotation DOFs as in the nonlinear TL warping path (χ rows/cols do not receive extra geometric stiffness terms). See [`JOB_INPUT_BEAM_WARPING.md`](JOB_INPUT_BEAM_WARPING.md) (*Linear buckling with warping*).
...
| `logs/run_manifest.json` | Written at successful job end: **`wall_time_sec`**, **`git_commit`** (best-effort), Python / **NumPy** / **SciPy** versions, **`simulation_settings_txt_sha256`**, resolved **`simulation_settings`** dict, and paths to **`newton_history.csv`**, **`inner_solve_history.csv`**, **`primary_summary.csv`**, and **`primary_artifacts.json`** (Sections 2–5 primary index) when present. |
