...
def test_modal_buckling_nonlinear_static_prestress_smoke():
    ...
    try:
        runner = ModalSimulationRunner(settings=settings, job_name="nl_prestress_smoke")
        runner.run()
...
def test_modal_buckling_linear_element_rejects_nonlinear_prestress():
    ...
    try:
        runner = ModalSimulationRunner(settings=settings, job_name="reject_nl")
        with pytest.raises(RuntimeError, match="Modal simulation aborted"):
            runner.run()
    finally:
...
def test_modal_buckling_nonlinear_static_with_linear_twins_smoke():
    ...
    try:
        runner = ModalSimulationRunner(settings=settings, job_name="nl_twins_smoke")
        runner.run()
        lam = np.asarray(runner.secondary_results["global"]["buckling_load_factors"], dtype=np.float64).ravel()
        assert np.all(np.isfinite(lam))
        assert np.any(lam > 0)
