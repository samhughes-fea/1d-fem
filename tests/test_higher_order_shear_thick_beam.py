"""
Higher-order shear (Phase 2b): thick beam (L/h small); compare Reddy vs Timoshenko tip deflection.
"""
...
def test_higher_order_shear_thick_beam():
    """Thick beam (L/h=3): single element cantilever; Reddy and Timoshenko both run; tip deflection differs from thin limit."""
    L = 0.3
    h = 0.1
    E, G = 2.1e11, 8.1e10
    b = 0.05
    ...
    job_results_dir = os.path.join(temp_dir, "thick_beam")
    ...
    def solve_cantilever(element_type_name):
        element_dictionary = {
            "ids": np.array([0]),
            "connectivity": np.array([[0, 1]]),
            "types": np.array([element_type_name]),
            "integration_orders": orders["integration_orders"],
        }
    ...
    try:
        u_y_reddy = solve_cantilever("LinearReddyBeamElement3D")
        u_y_timo = solve_cantilever("LinearTimoshenkoBeamElement3D")
        assert np.isfinite(u_y_reddy) and np.isfinite(u_y_timo)
        assert abs(u_y_reddy) > 1e-12 and abs(u_y_timo) > 1e-12
        assert u_y_reddy != u_y_timo or np.isclose(u_y_reddy, u_y_timo, atol=1e-9), (
            "Thick beam: Reddy and Timoshenko may differ in tip deflection"
        )
    finally:
        import shutil
