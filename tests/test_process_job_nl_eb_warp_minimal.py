"""
End-to-end: process_job with NonlinearEulerBernoulliBeamElement3D and [warping]=1.
...
def _write_minimal_nl_eb_warp_job(job_dir: Path) -> None:
    ...
    (job_dir / "element.txt").write_text(
        ...
        "0             0        1        NonlinearEulerBernoulliBeamElement3D   3                 3                 3                 0                 0                 3                 2                 1\n",
        encoding="utf-8",
    )
...
def test_process_job_minimal_nl_eb_warp_converges(tmp_path: Path) -> None:
    job_dir = tmp_path / "job_minimal_nl_eb_warp"
    _write_minimal_nl_eb_warp_job(job_dir)
    res_dir = setup_job_results_directory("test_minimal_nl_eb_warp")
    jt: dict = {}
    je: dict = {}
    process_job(
        str(job_dir),
    ...
    # Nonlinear run should write primary results
